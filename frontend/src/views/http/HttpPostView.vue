...
import { useHttpPostStore, type FormDataEntry } from '../../stores/httpPostStore'

// --- 使用 Post Store ---
const httpPostStore = useHttpPostStore()

// --- 代理计算属性 ---
const requestUrl = computed({
  get: () => httpPostStore.request.url,
  set: (v) => httpPostStore.setRequestUrl(v),
})
const requestParams = computed({
  get: () => httpPostStore.request.params,
  set: (v) => httpPostStore.setParams(v),
})
const requestHeaders = computed({
  get: () => httpPostStore.request.headers,
  set: (v) => httpPostStore.setHeaders(v),
})
const requestBodyType = computed({
  get: () => httpPostStore.request.bodyType,
  set: (v) => httpPostStore.setBodyType(v as 'json' | 'form-data'),
})
const requestBodyRaw = computed({
  get: () => httpPostStore.request.bodyRaw,
  set: (v) => httpPostStore.setBodyRaw(v),
})
const requestBodyFormData = computed({
  get: () => httpPostStore.request.bodyFormData,
  set: (value) => httpPostStore.setBodyFormData(value),
})
...
const addFormDataRow = () => {
  ...
  httpPostStore.setBodyFormData(newRows)
}
...
const sendRequest = async () => {
  ...
  resetUploadState()
  if (httpPostStore.request.bodyType === 'form-data') {
    uploadState.isUploading = true
  }
  const startTime = Date.now()
  try {
    const finalUrl = new URL(httpPostStore.request.url)
    httpPostStore.request.params.forEach((param) => {
      if (param.enabled && param.key) {
        finalUrl.searchParams.append(param.key, param.value as string)
      }
    })
    const headers: Record<string, string> = {}
    httpPostStore.request.headers.forEach((header) => {
    ...
    if (httpPostStore.request.bodyType === 'form-data') {
      ...
      httpPostStore.request.bodyFormData.forEach((item) => {
      ...
      uploadState.isUploading = false
      try {
        bodyData = JSON.parse(httpPostStore.request.bodyRaw)
      } catch (e) {
        bodyData = httpPostStore.request.bodyRaw
    ...
    const res = await axios.post(finalUrl.toString(), bodyData, axiosConfig)
...
.method-tag.put {
  background-color: #61affe; /* 之前我们定义好的蓝色 */
}
/* URL 输入栏 */
.url-bar {
  display: flex;
...
.form-data-editor .kv-input-wrapper {
  flex: 1; /* 使用 flex: 1 自动且平均地分配空间 */
  min-width: 150px; /* 添加一个最小宽度，防止在极窄屏幕下被压得太扁 */
}
