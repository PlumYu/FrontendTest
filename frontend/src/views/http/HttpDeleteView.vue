...
import { useHttpDeleteStore } from '../../stores/httpDeleteStore'

// --- 使用 DELETE Store ---
const httpDeleteStore = useHttpDeleteStore()

// --- 代理计算属性，确保通过 Action 更新 Store ---
const requestUrl = computed({
  get: () => httpDeleteStore.request.url,
  set: (value) => httpDeleteStore.setRequestUrl(value),
})
const requestParams = computed({
  get: () => httpDeleteStore.request.params,
  set: (value) => httpDeleteStore.setParams(value),
})
const requestHeaders = computed({
  get: () => httpDeleteStore.request.headers,
  set: (value) => httpDeleteStore.setHeaders(value),
})
...
const sendRequest = async () => {
  ...
  try {
    const finalUrl = new URL(httpDeleteStore.request.url)
    httpDeleteStore.request.params.forEach((param) => {
      if (param.enabled && param.key) {
        finalUrl.searchParams.append(param.key, param.value as string)
      }
    })
    const headers: Record<string, string> = {}
    httpDeleteStore.request.headers.forEach((header) => {
      if (header.enabled && header.key) {
        headers[header.key] = header.value
      }
    })
    const res = await axios.delete(finalUrl.toString(), { headers })
    response.value = {
      status: res.status,
      data: res.data,
      headers: res.headers,
